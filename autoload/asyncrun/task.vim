...
let g:asyncrun#task#shell = ''
let g:asyncrun#task#shellcmdflag = ''
...
function! s:task_start(task, cmd, opts) abort
	...
	if g:asyncrun#task#shell != ''
		let l:shell = g:asyncrun#task#shell
		let l:shellcmdflag = g:asyncrun#task#shellcmdflag
	endif
	if get(a:opts, 'shell', '') != ''
...
" new task object
"----------------------------------------------------------------------
function! asyncrun#task#new(callback, name)
...
if 0
	function! s:my_cb(task, event, data) abort
		...
		cbottom
	endfunc
	let t1 = asyncrun#task#new(function('s:my_cb'), "test task A")
	let t2 = asyncrun#task#new(function('s:my_cb'), "test task B")
	cexpr ""
	" call t1.start('dir', {})
	if s:windows == 0
